...
- **空间可视化**：通过中国地图以色块形式直观展示各区域舆情热度分布
...
* **数据持久层 (Database)**：
    * **数据库结构**：[`init.sql`](./Data_Frontend/init.sql) —— 系统数据库具有以下特点：
        1. 以`hot_things` (热点事件)表为核心，存储事件基本信息、热度值和统计指标、`population_values` (人群价值观)、`typical_radar` (雷达图数据) 等核心表结构设计。
        2. 通过多表关联设计，实现了事件-地理位置、事件-人群构成、事件-用户情绪、事件-互动数据等多维关系映射。
        3. `typical posts`、`typical radar`等表支持典型帖子价值观分布和雷达图数据特征提取，`population compositionpopulation values`等表实现人群价值观分析
